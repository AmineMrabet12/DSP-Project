...
class Environment:
    ...
    def update_airflow_config(self):
        ...
        """
        source_file = os.path.join(self.base_path, self.config["relative_paths"]["git_airflow_config"])
        target_file = self.config["local_paths"]["airflow_config"]
        try:
            with open(source_file, 'r') as file:
                data = file.read()
            pattern = r"(/\S+/airflow)"
            updated_config = re.sub(pattern, self.config["local_paths"]["airflow_install_dir"], data, flags=re.IGNORECASE)

            new_sql_alchemy_value = "sqlite:///" + os.path.join(self.config["local_paths"]["airflow_install_dir"], "airflow.db")
            updated_config = re.sub(r"(sql_alchemy_conn\s*=\s*)(.*)",rf"\1{new_sql_alchemy_value}", updated_config)

            new_dags_folder = os.path.join(self.base_path.replace("environment", ""), "airflow", "dags")
            updated_config = re.sub(r"(dags_folder\s*=\s*)(.*)",rf"\1{new_dags_folder}", updated_config)
