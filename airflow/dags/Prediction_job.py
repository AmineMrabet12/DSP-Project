from airflow import DAG
from airflow.operators.python import PythonOperator
from airflow.exceptions import AirflowSkipException
from airflow.utils.dates import days_ago
import os
import requests
import pandas as pd
import json
from dotenv import load_dotenv

load_dotenv()

# Set up data paths
data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../data/")
GOOD_DATA_FOLDER = os.path.join(data_path, "good-data")
PROCESSED_FILES_PATH = os.path.join(data_path, "processed_files.json")

# Load the API URL from environment variables
MODEL_API_URL = os.getenv('FASTAPI_PREDICT_URL')

def load_processed_files():
    """Load processed files from JSON if they exist."""
    if os.path.exists(PROCESSED_FILES_PATH):
        try:
            with open(PROCESSED_FILES_PATH, 'r') as file:
                content = file.read().strip()
                return json.loads(content) if content else {}
        except json.JSONDecodeError:
            print("Error decoding JSON from processed files.")
            return {}
    return {}

def save_processed_files(files):    
    """Save processed files to JSON."""
    with open(PROCESSED_FILES_PATH, 'w') as file:
        json.dump(files, file)

def check_for_new_data(**kwargs):
    """Check for new or modified CSV files in the good data folder."""
    files = os.listdir(GOOD_DATA_FOLDER)
    new_files = [
        f for f in files 
        if os.path.isfile(os.path.join(GOOD_DATA_FOLDER, f)) and f.endswith('.csv')
    ]
    
    processed_files = load_processed_files()
    files_to_process = []

    for file in new_files:
        file_path = os.path.join(GOOD_DATA_FOLDER, file)
        last_mod_time = os.path.getmtime(file_path)

        # Check if the file is new or modified
        if file not in processed_files or processed_files[file] != last_mod_time:
            files_to_process.append(file)

    if files_to_process:
        print('Found new or modified data:', files_to_process)
        kwargs['ti'].xcom_push(key='new_files', value=files_to_process)

        # Update processed files tracking
        for file in files_to_process:
            processed_files[file] = os.path.getmtime(os.path.join(GOOD_DATA_FOLDER, file))
        save_processed_files(processed_files)
    else:
        raise AirflowSkipException("No new or modified files found in good-data folder")

def make_predictions(**kwargs):
    """Make predictions using the new files."""
    new_files = kwargs['ti'].xcom_pull(key='new_files', task_ids='check_for_new_data')
    print("New files to process:", new_files)

    predictions = []
    if new_files:
        for file in new_files:
            file_path = os.path.join(GOOD_DATA_FOLDER, file)

            # Read the CSV file
            df = pd.read_csv(file_path)
            data_json = df.to_dict(orient='records')

            # Make predictions by sending data to the model API
            response = requests.post(MODEL_API_URL, json=data_json, headers={"X-Source": "airflow"})
            
            if response.status_code == 200:
                if "message" in response.json():
                    predictions.append(f"{response.json()['message']} for {file}")
                else:
                    predictions.append(response.json().get('predictions', "No predictions found"))
            else:
                predictions.append(f"Error: {response.status_code}")

    return predictions

default_args = {
    'owner': 'airflow',
    'start_date': days_ago(1),
    'depends_on_past': False,
    'retries': 0,
}

# Define the DAG
with DAG(
    'Prediction_job',
    default_args=default_args,
    schedule_interval='*/2 * * * *',
    catchup=False,
) as dag:

    check_for_new_data_task = PythonOperator(
        task_id='check_for_new_data',
        python_callable=check_for_new_data,
        provide_context=True,
    )

    make_predictions_task = PythonOperator(
        task_id='make_predictions',
        python_callable=make_predictions,
        provide_context=True,
    )

    check_for_new_data_task >> make_predictions_task
